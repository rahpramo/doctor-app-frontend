  ...
  return (
    <Container maxWidth="lg" sx={{ py: 6 }}>
      ...
      <Typography variant="body1" color="text.secondary" sx={{ mb: 4 }}>
        Our team of expert {speciality.toLowerCase()} specialists is ready to provide you with the best care.
      </Typography>

      {/* Results Count */}
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
        Found {filteredDoctors.length} {speciality.toLowerCase()} specialist{filteredDoctors.length !== 1 ? 's' : ''}
      </Typography>
      ...
      {filteredDoctors.length > 0 ? (
        ...
        <Box sx={{ textAlign: "center", py: 8 }}>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
            We don't have any doctors specializing in {speciality.toLowerCase()} at the moment.
          </Typography>
          <Button 
            variant="contained" 
...
const DoctorCard = React.memo(({ doctor, onClick }) => {
  ...
  return (
    ...
    >
      <CardMedia
        component="img"
        image={imageError || !doctor.avatar ? `${strapiMediaUrl}/upload_area_8bc1382fd8.png` : doctor.avatar?.url}
        alt={doctor.name}
