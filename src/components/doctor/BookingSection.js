    ...
    try {
      ...
      if (result.success) {
        ...
        setSnackbar({
          ...
          message: `Booked ${
            selectedDoctor.name
          } at ${date.toLocaleString()} successfully!`,
          severity: "success",
        });
    ...
    <Box mt={4}>
      <Paper elevation={2} sx={{p: 3, borderRadius: 2}}>
        ...
        <Stack direction="row" spacing={1} sx={{mb: 3}} flexWrap="wrap">
          ...
          {selectedDoctor.fee && (
            <Chip label={`Fee: ${selectedDoctor.fee}`} color="secondary" />
          )}
        </Stack>

        <Typography variant="body1" color="text.secondary" sx={{mb: 3}}>
          Select a date and time for your appointment. Appointments must be made
